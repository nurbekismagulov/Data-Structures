...
private:
  ...
  // -------------------------------------------------------------------------
  void dfs(FlightGraph::Vertex node, map<string, bool>& visited, map<string,string>& parent){
    ...
    for(FlightGraph::EdgeItor it = successors.begin(); it != successors.end(); ++it){
      string nextairport = *((*it).dest());
      if(!(visited[nextairport])){
        parent[nextairport] = name;
        dfs((*it).dest(), visited, parent);
      }
    }
  }
};
